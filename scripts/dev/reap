...
SESSION_DOCKER="SERVERS"
SESSION_DEV="CODE"
ROOT_DIR="$HOME/Work/rei-auto-pilot"
...
if tmux has-session -t $SESSION_DEV 2>/dev/null; then
  ...
  tmux new-session -d -s $SESSION_DEV -n "frontend"
  tmux send-keys -t $SESSION_DEV:frontend "cd $ROOT_DIR/web" C-m
  tmux send-keys -t $SESSION_DEV:frontend "vim ." C-m

  tmux new-window -n "backend" -t $SESSION_DEV:
  tmux send-keys -t $SESSION_DEV:backend "cd $ROOT_DIR/reaper/" C-m
